...
if historical_total > 0:
    ...
    st.markdown(f"""
<div style='background-color:#0d3b1e; padding:15px; border-radius:10px; color:white; font-size:16px;'>
🔍 EV adoption in <b>{county}</b> is expected to show a <b>{trend} of {forecast_growth_pct:.2f}%</b> over the next 3 years.
</div>
""", unsafe_allow_html=True)
else:
...
if multi_counties:
    ...
    st.markdown(f"""
<div style='background-color:#0d3b1e; padding:15px; border-radius:10px; color:white; font-size:16px;'>
📊 Forecasted EV adoption growth over next 3 years — <b>{growth_sentence}</b>
</div>
""", unsafe_allow_html=True)
